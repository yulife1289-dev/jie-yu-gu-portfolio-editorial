...
function renderProject(slug){
  ...
  const cleanups=[...view.querySelectorAll('.case-carousel')].map(root=>bindCaseScrollGallery(root,p));
  state.reel={destroy:()=>cleanups.forEach(fn=>fn())};
}
...
function caseCarousel(project,images,section,number){
  ...
  return `<section class="case-carousel case-scroll-carousel reveal"><header><span>${String(number).padStart(2,'0')}</span><h2>${esc(section.zh)}<small>${esc(section.en)}</small></h2><p class="${ph?'placeholder':''}">${esc(section.copy)}</p></header><div class="case-scroll-gallery"><div class="case-scroll-track" tabindex="0" aria-roledescription="carousel" aria-label="${esc(section.zh)}圖片輪播">${images.map((im,i)=>`<button class="case-scroll-slide" data-global="${project.images.indexOf(im)}" aria-label="放大第 ${i+1} 張圖片"><img ${imageAttrs(im,number<4&&i===0)}></button>`).join('')}</div><span class="case-scroll-count" aria-live="polite">01 / ${String(images.length).padStart(2,'0')}</span></div></section>`;
}
function bindCaseScrollGallery(root,project){
  const track=root.querySelector('.case-scroll-track'),slides=[...root.querySelectorAll('.case-scroll-slide')],count=root.querySelector('.case-scroll-count');let raf=0,current=0;
